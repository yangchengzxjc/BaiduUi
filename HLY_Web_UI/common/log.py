...
class LogSignleton(object):
    ...
    def __new__(cls, log_config):
        ...
        if not hasattr(cls, 'instance'):
            ...
            # logpath=str(config.get('LOGGING', 'log_file'))
            # cls.instance.log_filename = str(logpath)
    ...
    def __config_logger(self):
        ...
        if self.logfile_log_on == 1: # 如果开启文件日志
            ...
            # rt_file_handler = RotatingFileHandler(self.log_filename, maxBytes=self.max_bytes_each, backupCount=self.backup_count,encoding='utf-8')
            rt_file_handler = TimedRotatingFileHandler(self.log_filename, when='midnight', interval=1,backupCount=self.backup_count)
            # rt_file_handler = TimedRotatingFileHandler(self.log_filename , "D", 1, self.backup_count)
            # rt_file_handler = TimedRotatingFileHandler(log_filename, "H", 1, backup_count)
            rt_file_handler.setFormatter(formatter)
            self.logger.addHandler(rt_file_handler)
            self.logger.setLevel(self.log_level_in_logfile)
logsignleton = LogSignleton("./config/log.config")
